...
HRESULT WINAPI QueryApiImpl( const GUID *classId, REFIID interfaceId, void **out )
{
    ...
    if (IsEqualGUID( classId, &CLSID_XErrorImpl ))
        return IXError_QueryInterface( x_error_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XGameImpl ))
    ...
    if (IsEqualGUID( classId, &CLSID_XGameInviteImpl ))
    ...
    if (IsEqualGUID( classId, &CLSID_XGameSaveImpl ))
        return IXGameSave3_QueryInterface( x_game_save_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XGameStreamingImpl ))
        return IXGameStreaming3_QueryInterface( x_game_streaming_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XGameUiImpl ))
        return IXGameUi4_QueryInterface( x_game_ui_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XLauncherImpl ))
        return IXLauncher_QueryInterface( x_launcher_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XNetworkingImpl ))
        return IXNetworking2_QueryInterface( x_networking_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XPackageImpl ))
        return IXPackage3_QueryInterface( x_package_impl, interfaceId, out );
    if (IsEqualGUID( classId, &CLSID_XPersistentLocalStorageImpl ))
    ...
    if (IsEqualGUID( classId, &CLSID_XSystemAnalyticsImpl ))
    ...
    if (IsEqualGUID( classId, &CLSID_XUserDeviceImpl ))
        return IXUserDevice2_QueryInterface( x_user_device_impl, interfaceId, out );
