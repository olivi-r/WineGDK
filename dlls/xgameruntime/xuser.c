...
struct x_user
{
    IXUser6 IXUser6_iface;
    IXUserGamertag IXUserGamertag_iface;
    IXUserDevice2 IXUserDevice2_iface;
    LONG ref;
};

static inline struct x_user *impl_from_IXUser6( IXUser6 *iface )
{
    return CONTAINING_RECORD( iface, struct x_user, IXUser6_iface );
}

static HRESULT WINAPI x_user_QueryInterface( IXUser6 *iface, REFIID iid, void **out )
{
    struct x_user *impl = impl_from_IXUser6( iface );
    ...
    {
        IXUser6_AddRef( *out = &impl->IXUser6_iface );
        return S_OK;
    }
...
static HRESULT WINAPI x_user_gamertag_QueryInterface( IXUserGamertag *iface, REFIID riid, void **out )
{
    ...
    return IXUser6_QueryInterface( &impl->IXUser6_iface, riid, out );
}

static ULONG WINAPI x_user_gamertag_AddRef( IXUserGamertag *iface )
{
    struct x_user *impl = impl_from_IXUserGamertag( iface );
    return IXUser6_AddRef( &impl->IXUser6_iface );
}

static ULONG WINAPI x_user_gamertag_Release( IXUserGamertag *iface )
{
    struct x_user *impl = impl_from_IXUserGamertag( iface );
    return IXUser6_Release( &impl->IXUser6_iface );
}
...
static inline struct x_user *impl_from_IXUserDevice2( IXUserDevice2 *iface )
{
    return CONTAINING_RECORD( iface, struct x_user, IXUserDevice2_iface );
}

static HRESULT WINAPI x_user_device_QueryInterface( IXUserDevice2 *iface, REFIID iid, void **out )
{
    struct x_user *impl = impl_from_IXUserDevice2( iface );

    TRACE( "iface %p, iid %s, out %p.\n", iface, debugstr_guid( iid ), out );

    if (IsEqualGUID( iid, &IID_IUnknown      ) ||
        IsEqualGUID( iid, &IID_IXUserDevice  ) ||
        IsEqualGUID( iid, &IID_IXUserDevice2 ))
    {
        IXUserDevice2_AddRef( *out = &impl->IXUserDevice2_iface );
        return S_OK;
    }
...
static ULONG WINAPI x_user_device_AddRef( IXUserDevice2 *iface )
{
    struct x_user *impl = impl_from_IXUserDevice2( iface );
    ULONG ref = InterlockedIncrement( &impl->ref );
    TRACE( "iface %p increasing refcount to %lu.\n", iface, ref );
...
static ULONG WINAPI x_user_device_Release( IXUserDevice2 *iface )
{
    struct x_user *impl = impl_from_IXUserDevice2( iface );
    ULONG ref = InterlockedDecrement( &impl->ref );
    TRACE( "iface %p decreasing refcount to %lu.\n", iface, ref );
...
static HRESULT WINAPI x_user_device_XUserFindForDevice( IXUserDevice2 *iface, const APP_LOCAL_DEVICE_ID *deviceId, XUserHandle *handle )
{
    FIXME( "iface %p, deviceId %p, handle %p stub!\n", iface, deviceId, handle );
    return E_NOTIMPL;
}

static HRESULT WINAPI x_user_device_XUserRegisterForDeviceAssociationChanged( IXUserDevice2 *iface, XTaskQueueHandle queue, void *context, XUserDeviceAssociationChangedCallback *callback, XTaskQueueRegistrationToken *token )
{
    FIXME( "iface %p, queue %p, context %p, callback %p, token %p stub!\n", iface, queue, context, callback, token );
    return E_NOTIMPL;
}

static BOOLEAN WINAPI x_user_device_XUserUnregisterForDeviceAssociationChanged( IXUserDevice2 *iface, XTaskQueueRegistrationToken token, BOOLEAN wait )
{
    FIXME( "iface %p, token %p, wait %d stub!\n", iface, &token, wait );
    return FALSE;
}

static HRESULT WINAPI x_user_device_XUserGetDefaultAudioEndpointUtf16( IXUserDevice2 *iface, XUserLocalId user, XUserDefaultAudioEndpointKind defaultAudioEndpointKind, SIZE_T endpointIdUtf16Count, WCHAR *endpointIdUtf16, SIZE_T *endpointIdUtf16Used )
{
...
static HRESULT WINAPI x_user_device_XUserRegisterForDefaultAudioEndpointUtf16Changed( IXUserDevice2 *iface, XTaskQueueHandle queue, void *context, XUserDefaultAudioEndpointUtf16ChangedCallback *callback, XTaskQueueRegistrationToken *token )
{
    FIXME( "iface %p, queue %p, context %p, callback %p, token %p stub!\n", iface, queue, context, callback, token );
    return E_NOTIMPL;
}

static BOOLEAN WINAPI x_user_device_XUserUnregisterForDefaultAudioEndpointUtf16Changed( IXUserDevice2 *iface, XTaskQueueRegistrationToken token, BOOLEAN wait )
{
    FIXME( "iface %p, token %p, wait %d stub!\n", iface, &token, wait );
    return FALSE;
}

static HRESULT WINAPI x_user_device_XUserFindControllerForUserWithUiAsync( IXUserDevice2 *iface, XUserHandle user, XAsyncBlock *async )
{
    FIXME( "iface %p, user %p, async %p stub!\n", iface, user, async );
    return E_NOTIMPL;
}

static HRESULT WINAPI x_user_device_XUserFindControllerForUserWithUiResult( IXUserDevice2 *iface, XAsyncBlock *async, APP_LOCAL_DEVICE_ID *deviceId )
{
    FIXME( "iface %p, async %p, deviceId %p stub!\n", iface, async, deviceId );
    return E_NOTIMPL;
}

static const struct IXUserDevice2Vtbl x_user_device_vtbl =
{
    x_user_device_QueryInterface,
    x_user_device_AddRef,
    x_user_device_Release,
    /* IXUserDevice/IXUserDevice2 methods */
...
IXUser6 *x_user_impl = &x_user.IXUser6_iface;
IXUserDevice2 *x_user_device_impl = &x_user.IXUserDevice2_iface;
