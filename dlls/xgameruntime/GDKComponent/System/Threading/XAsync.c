...
static HRESULT AllocStateNoCompletion( XAsyncBlock* asyncBlock, AsyncBlockInternal* internal, size_t contextSize )
{
    ...
    if ( queue == NULL )
    {
        if ( IXThreading_XTaskQueueGetCurrentProcessTaskQueue( x_threading_impl, &stateImpl->queue ) == FALSE )
        {
    ...
    {
        ((struct XTaskQueueObject*)queue)->headQueue->lpVtbl->AddRef( ((struct XTaskQueueObject*)queue)->headQueue );
