...
static HRESULT WINAPI user_RefreshXstsToken( IUser *iface )
{
    ...
    hr = IUser_RequestXToken( iface, L"xsts.auth.xboxlive.com", L"/xsts/authorize", "http://xboxlive.com", props, (IUnknown **)&object );
    free( props );
    if (FAILED(hr)) return hr;
...
static INT32 WINAPI x_user_XUserCompare( IXUser6 *iface, XUserHandle user1, XUserHandle user2 )
{
    ...
    return ((struct user *)user1)->xuid == ((struct user *)user2)->xuid ? 0 : 1;
}
...
static HRESULT WINAPI XUserAddProvider( XAsyncOp op, const XAsyncProviderData *data )
{
    ...
    if (FAILED(hr = QueryApiImpl( &CLSID_XThreadingImpl, &IID_IXThreading, (void **)&xthreading ))) return hr;
    context = (struct XUserAddContext *)data->context;

    switch (op)
    {
        case XAsyncOp_Begin:
            hr = IXThreading_XAsyncSchedule( xthreading, data->async, 0 );
            break;
        ...
        case XAsyncOp_DoWork:
            ...
            IXThreading_XAsyncComplete( xthreading, data->async, hr, sizeof(XUserHandle) );
            hr = S_OK;
            break;
...
static HRESULT WINAPI x_user_XUserAddAsync( IXUser6 *iface, XUserAddOptions options, XAsyncBlock *async )
{
    ...
    hr = IXThreading_XAsyncBegin( xthreading, async, context, NULL, "XUserAddAsync", XUserAddProvider );
    IXThreading_Release( xthreading );
...
static HRESULT WINAPI x_user_XUserAddResult( IXUser6 *iface, XAsyncBlock *async, XUserHandle *newUser )
{
    ...
    if (FAILED(hr = QueryApiImpl( &CLSID_XThreadingImpl, &IID_IXThreading, (void **)&xthreading ))) return hr;
    hr = IXThreading_XAsyncGetResult( xthreading, async, x_user_XUserAddAsync, sizeof(XUserHandle), newUser, NULL );
    IXThreading_Release( xthreading );
    return hr;
}
...
static HRESULT WINAPI XUserGetTokenAndSignatureProvider( XAsyncOp op, const XAsyncProviderData *data )
{
    ...
    switch (op)
    {
        ...
        case XAsyncOp_DoWork:
            IXThreading_XAsyncComplete( xthreading, data->async, E_NOTIMPL, 0 );
            hr = S_OK;
            break;
...
static HRESULT WINAPI x_user_XUserGetTokenAndSignatureAsync( IXUser6 *iface, XUserHandle user, XUserGetTokenAndSignatureOptions options, const CHAR *method, const CHAR *url, SIZE_T headerCount, const XUserGetTokenAndSignatureHttpHeader *headers, SIZE_T bodySize, const void *bodyBuffer, XAsyncBlock *async )
{
    ...
    if (FAILED(hr = IXUser6_XUserDuplicateHandle( iface, user, &context->user )))
    {
        IXThreading_Release( xthreading );
        return hr;
    }
    ...
    hr = IXThreading_XAsyncBegin( xthreading, async, context, NULL, "XUserGetTokenAndSignatureAsync", XUserGetTokenAndSignatureProvider );
    IXThreading_Release( xthreading );
    if (FAILED(hr)) free( context );
    return hr;
}
...
static HRESULT WINAPI x_user_XUserGetTokenAndSignatureUtf16Async( IXUser6 *iface, XUserHandle user, XUserGetTokenAndSignatureOptions options, const WCHAR *method, const WCHAR *url, SIZE_T headerCount, const XUserGetTokenAndSignatureUtf16HttpHeader *headers, SIZE_T bodySize, const void *bodyBuffer, XAsyncBlock *async )
{
    ...
    TRACE( "iface %p, user %p, options %d, method %hs, url %hs, headerCount %Iu, headers %p, bodySize %Iu, bodyBuffer %p, async %p.\n", iface, user, options, method, url, headerCount, headers, bodySize, bodyBuffer, async );
    ...
    hr = IXThreading_XAsyncBegin( xthreading, async, context, NULL, "XUserGetTokenAndSignatureUtf16Async", XUserGetTokenAndSignatureUtf16Provider );
    IXThreading_Release( xthreading );
    if (FAILED(hr)) free( context );
    return hr;
}
